...
class StockMasterTable
{
   protected $tableGateway;
	...
	public function getStockBodyStyle()
	{
		...
		$select->where("status= '1' AND show_on_site = 1");
	...
	public function getStockcnt($strWhere)
	{
		...
		$rowset = $statement->execute();
         if ($rowset->count()) {
         $rows = new ResultSet();
         $result = $rows->initialize($rowset)->toArray();
		 return $result[0]['cnt'];
		}
		return 0;
	}
...
//echo  $select->getSqlString(); die;
	...
	public function getStockList($strWhere="",$strOrder="")
	{
		...
		$select2->where("sm.status = '1' AND sm.show_on_site = 1");
		if($strWhere != "")		$select2->where($strWhere);
		
		$select1 = $sql->select();

		$select1->from(array("sm"=>"stock_master"));
		$select1->join(array("mmv"=>"make_model_variant"),'sm.mmv_id=mmv.mmv_id',array('bod_style','noofcylinders','engine_cc','aircondition','alloy_wheels','leather_seats'));
		$select1->join(array("si"=>"stock_images"),'sm.stock_id=si.stock_id',array("image_name" => new Expression("GROUP_CONCAT(image_name)"),
				"is_profile_img" => new Expression("GROUP_CONCAT(is_profile_img)"),"image_title" => new Expression("GROUP_CONCAT(image_title)")),$select1::JOIN_LEFT);
		...
		$select1->join(array("sr"=>"showroom_master"),'sm.showroom_id=sr.showroom_id',array("showroom_name",
							"land_mark","locality","city","state","pin_code","phone_1"));
		$select1->where("sm.status = '1' AND sm.show_on_site = 1");
		...
		$select->from(array("sm"=>"stock_master"));
		$select->join(array("mmv"=>"make_model_variant"),'sm.mmv_id=mmv.mmv_id',array('bod_style','noofcylinders','engine_cc','aircondition','alloy_wheels','leather_seats'));
		$select->join(array("si"=>"stock_images"),'sm.stock_id=si.stock_id',array("image_name" => new Expression("GROUP_CONCAT(image_name)"),
				"is_profile_img" => new Expression("GROUP_CONCAT(is_profile_img)"),"image_title" => new Expression("GROUP_CONCAT(image_title)")),$select::JOIN_LEFT);
		...
		$select->join(array("sr"=>"showroom_master"),'sm.showroom_id=sr.showroom_id',array("showroom_name",
							"land_mark","locality","city","state","pin_code","phone_1"));
		$select->where("sm.status = '1' AND sm.show_on_site = 1");
		if($strWhere != "")		$select->where($strWhere);
		if($strOrder != "")		$select->order($strOrder);
		$select->group("stock_id");
		
		$selectfinal = $sql->select()->from(array('result' => $select));
		$selectfinal1 = $sql->select()->from(array('result' => $select1));
		
		if($strWhere != "")	$selectfinal->combine ( $selectfinal1 );
		//showroom_id
		
		//echo  $selectfinal->getSqlString(); 
		$statement = $sql->prepareStatementForSqlObject($selectfinal);
	...
	public function getStockMakeModel()
	{
		$adapter = $this->tableGateway->getAdapter(); 
		$sql = new Sql($adapter);
		$select = $sql->select();
	...
	public function getStockDetail($stock_id)
	{
		...
		$select->join(array("mo"=>"model_master"),'mo.model_id=mmv.model_id',array("model_name"));
		$select->join(array("va"=>"variant_master"),'va.variant_id=mmv.variant_id',array("variant_name"));
		//$select->join(array("si"=>"stock_images"),'sm.stock_id=si.stock_id',array("image_name" => new Expression("GROUP_CONCAT(image_name)"),"image_title" => new Expression("GROUP_CONCAT(image_title)")),$select::JOIN_LEFT);
		$select->join(array("sr"=>"showroom_master"),'sm.showroom_id=sr.showroom_id',array("showroom_id","showroom_name",
							"land_mark","locality","city","state","pin_code","phone_1","email"));
		$select->where("sm.stock_id = $stock_id AND show_on_site = 1");
		$select->group("sm.stock_id");
		//echo  $select->getSqlString(); die;
		$statement = $sql->prepareStatementForSqlObject($select);
	...
	public function getStockImages($stock_id)
	{
		$adapter = $this->tableGateway->getAdapter(); 
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array("sm"=>"stock_images"));
		$select->where("sm.stock_id = $stock_id");
		$select->order('dis_order');
		$statement = $sql->prepareStatementForSqlObject($select);
